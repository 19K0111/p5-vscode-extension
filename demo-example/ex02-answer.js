    ...
    draw() {
        ...
        for (let y = 0; y < this.tiles.length; y++) {
            for (let x = 0; x < this.tiles[0].length; x++) {
                if (this.tiles[y][x] == 0) {
                    fill("#000000");
                } else if (this.tiles[y][x] == y * this.tiles.length + x + 1) {
...
function mousePressed() {
    if (!stop_flag) {
        ...
        // BEGIN
        if (environment.state.isMovable(x, y) && !environment.state.checkSuccess()) { // 穴が(x, y)に移動可能で、パズルが完成していないとき
...
async function automaticallyMove() {
    if (sp.solve(environment.state, 40)) {
        ...
        for (let s of sp.getSolution().reverse()) {
            ...
            // s = {x: number, y: number}
            console.log(s);
            if (s.x == 0 && s.y == 1/* 適切な条件に書き換える、以下同様 */) {
